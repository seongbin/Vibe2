...
function on_mouse_lbtn_up(x, y) { toggle.lbtn_up(x, y); seekbar.lbtn_up(x, y); rating.lbtn_up(x, y); volume.lbtn_up(x, y); buttons.lbtn_up(x, y); }
function on_mouse_leave() { toggle.leave(); rating.leave(); buttons.leave(); }
function on_mouse_move(x, y) { toggle.move(x, y); albumart.move(x, y); seekbar.move(x, y); rating.move(x, y); volume.move(x, y); buttons.move(x, y); }
...
function on_size() { panel.size(); albumart.size(); toggle.size(); seekbar.size(); rating.size(); buttons.update(); volume.size(); esl.size(); window.MaxHeight = panel.bs * 5; window.MinHeight = panel.bs * 3; window.minWidth = panel.bs * 25; }
function on_paint(gr) { panel.paint(gr); albumart.paint(gr); toggle.paint(gr); seekbar.paint(gr); rating.paint(gr); volume.paint(gr); buttons.paint(gr); }
