...
function _rating() {
	...
	this.size = function () {
		...
		this.x = Math.min(Math.max(albumart.x * 2 + albumart.w + tw, albumart.x * 2 + albumart.w + aw), seekbar.x - panel.bs - LM * 2);
	...
	this.paint = function (gr) {
		if (panel.metadb) {
			...
			if (this.get_rating() < 5) {
				...
				char = this.hover ? chars.heart_break : chars.heart_on;
			}
...
function _volume() {
	...
	this.size = function () { this.h = panel.row_height * 0.25; this.x = bx2 + panel.bs * 4 + LM; this.y = panel.h * 0.5 - this.h * 0.5; this.w = panel.bs * 3; this.mw = this.h * 1.25; this.mh = this.h * 1.25; }
...
buttons.update = function () {
  ...
  buttons.buttons.shuffle = new _button(bx1 - panel.bs * 2.75, by1, panel.bs, panel.bs, { char : chars.shuffle, colour : (plman.PlaybackOrder == 4) ? panel.colours.highlight : panel.colours.blend }, { char : chars.shuffle, colour : panel.colours.text }, function () { buttons.shuffle(); }, '');
  buttons.buttons.prev = new _button(bx1 - panel.bs * 1.5, by1, panel.bs, panel.bs, { char : chars.prev, colour : panel.colours.blend }, { char : chars.prev, colour : panel.colours.text }, function () { fb.Prev(); }, '');
  buttons.buttons.play = new _button(bx1, by1, panel.bs, panel.bs, { char : fb.IsPlaying ? !fb.IsPlaying || fb.IsPaused ? chars.play : chars.pause : chars.stop, colour : _n(DetermineTextColour(panel.colours.background)), bg : DetermineTextColour(panel.colours.background) }, { char : fb.IsPlaying ? !fb.IsPlaying || fb.IsPaused ? chars.play : chars.pause : chars.stop, colour : _n(DetermineTextColour(panel.colours.background)), bg : DetermineTextColour(panel.colours.background) }, function () { fb.PlayOrPause(); }, '');
  buttons.buttons.next = new _button(bx1 + panel.bs * 1.5, by1, panel.bs, panel.bs, { char : chars.next, colour : panel.colours.blend }, { char : chars.next, colour : panel.colours.text }, function () { fb.Next(); }, '');
	buttons.buttons.repeat = new _button(bx1 + panel.bs * 2.75, by1, panel.bs, panel.bs, { char : (plman.PlaybackOrder == 2) ? chars.repeat_on : chars.repeat_off, colour : (plman.PlaybackOrder == 4 || plman.PlaybackOrder == 2 || plman.PlaybackOrder == 1) ? panel.colours.highlight : panel.colours.blend }, { char : (plman.PlaybackOrder == 2) ? chars.repeat_on : chars.repeat_off, colour : panel.colours.text }, function () { buttons.repeat() }, '');
...
function on_mouse_lbtn_up(x, y) { seekbar.lbtn_up(x, y); rating.lbtn_up(x, y); volume.lbtn_up(x, y); picker.lbtn_up(x, y); buttons.lbtn_up(x, y); }
function on_mouse_leave() { rating.leave(); picker.leave(); buttons.leave(); }
function on_mouse_move(x, y) { albumart.move(x, y); seekbar.move(x, y); rating.move(x, y); volume.move(x, y); picker.move(x, y); buttons.move(x, y); }
